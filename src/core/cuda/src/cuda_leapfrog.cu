...
#include "cpu/include/cpu_shake.h"
...
real_t debug_force_component(force_accum_t value) {
...
__global__ void calc_leapfrog_kernel(
    ...
    velocities[i].x = (velocities[i].x - dvel_x * winv_i * dt) * scale;
    velocities[i].y = (velocities[i].y - dvel_y * winv_i * dt) * scale;
    velocities[i].z = (velocities[i].z - dvel_z * winv_i * dt) * scale;
...
void calc_leapfrog_host() {
    ...
    // Use the sequential SHAKE path for Fortran-compatible constraint ordering.
    if (host.n_shake_constraints > 0) {
        calc_shake_constraints(host.coords->cpu_data_p, host.xcoords->cpu_data_p);
        auto& velocities = host.velocities->cpu_data_p;
        auto& coords = host.coords->cpu_data_p;
        auto* xcoords = host.xcoords->cpu_data_p;
