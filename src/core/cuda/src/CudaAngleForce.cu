...
__device__ double to_radians_device(double degrees) {
...
double calc_angle_forces_host(int start, int end) {
    ...
    if (!is_initialized) {
        is_initialized = true;
        cudaMalloc((void **)&d_angles, n_angles * sizeof(angle_t));       
        cudaMalloc((void **)&d_coords, n_atoms * sizeof(coord_t));
        cudaMalloc((void **)&d_cangles, n_cangles * sizeof(cangle_t));
        cudaMalloc((void **)&d_dvelocities, n_atoms * sizeof(dvel_t));
        cudaMalloc((void **)&d_energy_sum, sizeof(double));
    }
