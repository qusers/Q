...
class WaterBoundaryForce {
   public:
    ...
    void init(Context& ctx, const Temperature& temperature, const std::vector<double>& restart_theta_corr);
   ...
   private:
    ...
    void build_wshells(Context& ctx, double crgQtot, const std::vector<double>& restart_theta_corr);
};