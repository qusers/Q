...
!  Miha Purg, Irek Szeler, Mauricio Esguerra, Viktor Prypoten                  !
...
!-------------------------------------------------------------------------------
  ...
  !-----------------------------------------------------------------------
  !       Non-Equilibrium variables, used to run NEQ simulations
  !-----------------------------------------------------------------------
  
  
  logical :: equilibrium_simulation
  character(len=200) :: scaling_parameter
  real(8)			 :: L_sigmoid !steepness parameter for sigmoid
  real(8)			 :: previous_U, dU, dlambda
  real(8)			 :: dU_dlambda !localy estimated change in potential energy wrgt lambda
  real(8)			 :: work_accumulated
...
9       format ('Pressure = ',f10.3,'  bar')
    ...
    if (.not. prm_open_section('lambda_scaling', infilename)) then
		...
		write(*,'(a)') 'No lambda scaling schedule provided, running equilibrium simulations'
	else
		equilibrium_simulation = .false.
		write(*,'(a)') 'using NEQ lambda scaling'

		if (prm_get_string_by_key('scaling_parameter', scaling_parameter)) then
			print *, "The value of scaling_parameter is:", scaling_parameter
		else
			print *, "No scaling parameter provided, using default"
			scaling_parameter = 'sigmoidal'
		end if

		if (prm_get_real8_by_key('L_sigmoid', L_sigmoid)) then
			print *, "The value of L_sigmoid value is:", L_sigmoid
		else
			print *, "Default steepness of 8 used"
			L_sigmoid = 8.0 !default steepness value
		end if

	end if !section lambda scaling
...
subroutine pot_energy
  ...
  integer                                 :: is, j
  real, allocatable :: old_lambda(:)
  real :: ratio, scaled_ratio,d_ratio,scaled_d_ratio
  real(8) :: L
#if defined (PROFILING)
...
#endif
  ! Allocate once before the MD loop
  allocate(old_lambda(nstates))
  ! --- reset all energies
  ...
  old_lambda(:) = EQ(:)%lambda
  
  ratio=real(istep) / real(nsteps)
  d_ratio=real(istep+1) / real(nsteps)

  if (scaling_parameter=='sigmoidal') then
    call sigmoid_rescale(ratio, L_sigmoid, scaled_ratio)
	call sigmoid_rescale(d_ratio, L_sigmoid, scaled_d_ratio)
    ratio = scaled_ratio
	d_ratio=scaled_d_ratio
  end if
	
  do istate = 1, nstates
  !this is a cludge to take a lambda state (say 1,0) and then scale the first component by the ratio, and the 2nd component is just the inverse
    if (istate==1) then
      EQ(istate)%lambda = old_lambda(istate) * (1-ratio) + (1-old_lambda(istate))*ratio
	  dlambda = (old_lambda(istate) * (1-d_ratio) + (1-old_lambda(istate))*d_ratio)-EQ(istate)%lambda
	else
	  EQ(istate)%lambda = 1 - EQ(1)%lambda
	end if
	
	if (equilibrium_simulation) then
	  EQ(istate)%lambda = old_lambda(istate)
	end if
  ...
  !gather the dU_dlambda value. Originally this has been sign flipped, so we just sign flip it again 
  ...
  !restore the old lambdas
  
  EQ(:)%lambda = old_lambda(:)
end subroutine pot_energy

!sigmoidal rescaling for switching. this is normalised from 0 to 1
subroutine sigmoid_rescale(u, L, snorm)
    implicit none
    ! Arguments
    real, intent(in)  :: u      ! input value in [0,1]
    real(8), intent(in)  :: L      ! steepness parameter
    real, intent(out) :: snorm  ! output rescaled value in [0,1]

    ! Local variables
    real :: s, s0, s1

    ! Compute raw sigmoid
    s  = 1.0 / (1.0 + exp(-(2.0*u - 1.0)*L))

    ! Sigmoid values at endpoints
    s0 = 1.0 / (1.0 + exp(+L))
    s1 = 1.0 / (1.0 + exp(-L))

    ! Normalize so that u=0 -> 0 and u=1 -> 1
    snorm = (s - s0) / (s1 - s0)
end subroutine sigmoid_rescale
...
subroutine write_out
  ...
  integer                                 ::      i,istate
  real, allocatable :: old_lambda(:)
  real :: ratio, scaled_ratio

  ! Allocate 
  allocate(old_lambda(nstates))
  
!this is a loop for simpler reporting and restoring
...
!if the NEQ schedule is used, write the total work accumulated
write(*,'(A,I0,A,E24.16,A,E24.16,E24.16)') 'At step ', istep, ', work accumulated was ', work_accumulated, ' and dU and dlambda were ',dU_dlambda, dlambda 
!restore the old lambdas
EQ(:)%lambda = old_lambda(:)
