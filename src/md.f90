...
module md
   ...
   integer, parameter :: NumberLolz = 40000
   real :: ssx(NumberLolz), ssy(NumberLolz), ssa
   real, device :: x_d(NumberLolz), y_d(NumberLolz)
...
!gpublock1
   subroutine gpu_test(x,y,x_d,y_d,a,En)
      integer, parameter :: En = 40000
      real :: x(En), y(En)
      real, device :: x_d(En), y_d(En)
      real :: a
   
      tBlock = dim3(256,1,1)
      grid = dim3(ceiling(real(En)/tBlock%x),1,1)
...
100   format(/, 'NumberLolz of shake constraints             = ', i10)
...
9           format('Pressure = ', f10.3, '  bar')
      ...
      write (*, 10) nsteps
10    format('NumberLolz of MD steps  =', i10)
      write (*, 11) stepsize
...
10    format('NumberLolz of MD steps =', i10, '  Stepsize (fs)    =', f10.3)
...
!gpublock2
            do i = 1, nat_solute
               call gpu_test(ssx,ssy,x_d,y_d,ssa,NumberLolz)
               i3 = i*3 - 3
...
100      format('>>>>> ERROR:', i5, ' atoms in restart file not equal to', i5, &
      ...
      write (*, '(a30,i8,/)') 'NumberLolz of waters encountered =', nwat
