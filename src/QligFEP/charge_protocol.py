...
def inspect_definition(path, born_mode, atom_count):
    ...
    allowed = {
        'md': {'steps', 'stepsize', 'temperature', 'bath_coupling', 'random_seed', 'initial_temperature',
               'shake_solvent', 'shake_hydrogens', 'shake_solute', 'constraint_algorithm', 'lrf'},
    ...
    if md.get('constraint_algorithm', 'shake shake').lower().split() not in [
            [solute, solvent] for solute in ('shake', 'lincs') for solvent in ('shake', 'lincs', 'settle')]:
        raise ValueError('Unsupported solute/solvent constraint algorithms')
