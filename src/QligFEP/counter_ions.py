"""Place counter-ions on a sphere by minimizing Coulomb repulsion.

Used in charge-changing FEP perturbations to neutralize the water leg
of the thermodynamic cycle within Q's spherical boundary conditions.
"""
...
def energy_and_grad(theta, phi):
    ...
    projected onto the spherical basis.
...
def minimize_coulomb_on_sphere(
    ...
    """Distribute Q point charges on a sphere to minimize Coulomb repulsion.

    Args:
        Q: Number of charges to place.
        radius: Radius of the sphere.
        center: (3,) array-like, center of the sphere.
        steps: Maximum optimization steps.
        seed: Random seed for reproducibility.
        verbose: Print optimization progress.

    Returns:
        xyz: (Q, 3) array of Cartesian coordinates for the placed charges.
    """
    ...
    for k in range(steps):
        # project to tangent plane (F = -nabla E)
        ...
        V = dir_t * (step_scale * clip)[:, None]  # downhill since F = -nabla E
        ...
        for _ in range(backtrack_max):
            ...
            # directional decrease prediction
        ...
        if verbose and (k % verbose_every == 0 or k == steps - 1):
            print(
                f"iter {k:5d} | E={E:.8f} | eta={step_scale:.5f} "
                f"| ||Ft||_max={max_norm:.3e} | ||Ft||_rms={rms_norm:.3e}"
            )
