...
#: PyMOL writes some hydrogens and both capping residues under names the Q
...
#: Flanking residue PyMOL attaches on either side of the mutated one.
_TRIPEPTIDE_FLANKS = {"A": "ala", "G": "gly"}
...
class QresFEP:
    ...
    def __init__(
        ...
        self.seeds = list(seeds) if seeds else list(range(1, self.replicates + 1))
    ...
    def _write_reference_peptide(self) -> None:
        ...
        synthetic Ala/Gly flanks -- and capped with ACE and NME. PyMOL does the
        capping, since it knows the geometry to attach.
        """
        ...
        # PyMOL attaches the C-terminal cap to whichever atom it is given by index,
        # so the backbone carbonyl carbon of the last residue is tracked while
        # writing. Anchoring anywhere else -- the alpha carbon, say -- grows the cap
        # off the side chain instead of off the main chain.
        cap_anchor = None
        serial = itertools.count(1)
        with open(complex_pdb, "w") as out:
            for number, residue in zip(target_numbers, source_residues):
                for atom in self.pdb[residue]:
                    atom[5], atom[6] = "", number
                    index = next(serial)
                    if number == target_numbers[-1] and atom[2] == "C":
                        cap_anchor = index
                    out.write(f"{pdb_parse_out(atom)}\n")

        if cap_anchor is None:
            raise MutationError(
                f"Q residue {source_residues[-1]} has no backbone C atom, so the reference "
                "peptide cannot be capped."
            )

        self._run_pymol_capping(cap_anchor)
        self._normalise_peptide_names(complex_pdb)

    def _run_pymol_capping(self, cap_anchor: int) -> None:
        """Run PyMOL to attach the flanking and capping residues."""
        substitutions = {
            "PDB": "complex.pdb",
            "COMPLEX": "complex",
            "TER": str(cap_anchor),
        ...
        if self.tripeptide_flanks in _TRIPEPTIDE_FLANKS:
            flank = _TRIPEPTIDE_FLANKS[self.tripeptide_flanks]
            substitutions["PRE"] = f"editor.attach_amino_acid('pk1', '{flank}')"
            substitutions["POST"] = (
                "cmd.edit('(COMPLEX`1)',None,None,None,pkresi=0,pkbond=0)\n"
                f"editor.attach_amino_acid('pk1', '{flank}')"
            )

        template = Path(CONFIGS["INPUT_DIR"]) / "tripeptide.pml"
        with open(template) as source, open(self.inputfiles / "tripeptide.pml", "w") as out:
            for line in source:
                # X (no flanks) and Z (native flanks) attach nothing extra.
                if self.tripeptide_flanks in ("X", "Z") and ("PRE" in line or "POST" in line):
                    continue
                out.write(replace(line, substitutions))

        result = subprocess.run(
            ["pymol", "-c", "tripeptide.pml"],
            cwd=self.inputfiles,
            capture_output=True,
            text=True,
        )
        if result.returncode != 0 or not (self.inputfiles / "tripeptide.pdb").exists():
            raise MutationError(
                "PyMOL could not build the reference peptide, which the tripeptide leg "
                f"needs (`pymol -c`).\n{result.stderr.strip()}"
            )

    def _normalise_peptide_names(self, complex_pdb: Path) -> None:
        """Rewrite PyMOL's atom and residue names to the ones Q's libraries use."""
        atoms = []
        for line in (self.inputfiles / "tripeptide.pdb").read_text().splitlines():
            if not line.startswith("ATOM"):
                continue
            for old, new in _PYMOL_NAME_FIXES:
                line = line.replace(old, new)
            atoms.append(line)

        self.system_size = len(atoms)
        with open(complex_pdb, "w") as out:
            out.write("\n".join(atoms) + "\n")
            self.counter_ions = self._append_counter_ions(out, len(atoms))
    ...
    def write_equilibration(self) -> None:
        ...
        configs = get_resfep_equilibration_configs(self.timestep, self.shell_radius)
    ...
    def write_production(self) -> list[list[str]]:
        ...
        config = get_production_config(
            ...
            interval_output=5,
            # Matches the equilibration: the mutation protocol couples the whole
            # system to one heat bath, unlike the ligand protocol.
        ...
        for stage, lambdas in enumerate(self.lambda_schedule(), start=1):
            ...
            for window in range(self.windows):
                ...
                content = render_md_input(
                    ...
                    lambda1=first,
                    lambda2=second,
                    trajectory_file=f"{basename}.dcd",
        ...
        with open(template) as source, open(target, "w") as out:
            for line in source:
                ...
                if line.strip() == "#CLEANUP" and self.to_clean:
                    out.write("rm -r " + " ".join(f"*{suffix}" for suffix in self.to_clean) + "\n")
