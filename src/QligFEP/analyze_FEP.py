...
class FepReader(object):
    ...
    def init_from_json(data_json: str, target_name: str, mapping_json: str):
        ...
        system = [k for k in data_dict.keys() if k != 'result'][0]
        feps = [k for k in data_dict[system].keys()]
    ...
    def read_fep_inputs(self):
        ...
        for fep in self.data[self.system].keys():
            ...
            try:
                ...
                logger.error(f'Failed to find the ligands for {fep}. Are you sure all calculations ran? Error: \n{e}')
    ...
    def load_experimental_data(self, exp_key: str):
        ...
        for fep in self.feps:
            ...
            if ddG is not None:
                ...
                self.data['result']['experimental'].update({fep: {
                    'from': _from,
                    'to': _to,
                    'value': ddG}})
            else:
    ...
    def create_ddG_plot(
        ...
        plt.text(*txt_position, text_body, fontsize=10, verticalalignment='bottom', horizontalalignment='right')
...
def main(args):
    ...
    fep_reader.populate_mapping_dictionary(method = args.method, output_file = args.json_file.replace('.json', '_ddG.json'))
