...
def parse(text):
    ...
    for key, count, size in [('state', nstates, 6), ('qatom', nqat, 11+nstates),
        ...
        result[key] = rows
    solver_lines = [line.split('=', 1)[1].lower().split() for line in lines
                    if line.startswith('Constraint algorithms =')]
    if solver_lines:
        if len(solver_lines) != 1 or solver_lines[0] not in [
                [solute, solvent] for solute in ('shake', 'lincs') for solvent in ('shake', 'lincs', 'settle')]:
            raise ValueError('Invalid native constraint algorithms')
        result['constraint_algorithms'] = solver_lines[0]
    return result
...
def validate_window(window, born_mode, log_path):
    ...
    explicit_solver = window['signature']['md'].get('constraint_algorithm')
    expected_solver = (explicit_solver or 'shake shake').lower().split()
    if ('constraint_algorithms' in audit or explicit_solver is not None) and audit.get('constraint_algorithms') != expected_solver:
        raise ValueError('Native constraint algorithms disagree with staged input')
