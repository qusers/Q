...
from env.settings import FF_DIR
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-pdb", "--pdb_file", dest='pdb_file', required=True, help="input protein to run qprep"
    )
    parser.add_argument(
        "-FF",
        "--forcefield",
...
def main(args: Optional[argparse.Namespace] = None, **kwargs) -> None:
    ...
    ff_lib_path = str(Path(FF_DIR) / f'{args.FF}.lib')
    ff_prm_path = str(Path(FF_DIR) / f'{args.FF}.prm')
    if args.cysbond != "":
        cysbonds = args.cysbond.split(',')
        cysbonds = '\n'.join([f'addbond {b.split('_')[0]} {b.split('_')[1]} y' for b in cysbonds])
