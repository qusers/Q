"""Check shared-constraint convergence without dynamics or charge forces."""
...
def test_native_shake_returns_only_when_all_final_constraints_converge(tmp_path):
    ...
    assert fields[0] == 'SHAKE_MAX' and len(fields) == 5
    _, sample, bond, residual, tolerance = fields
    assert 1 <= int(sample) <= 64 and 1 <= int(bond) <= 3
