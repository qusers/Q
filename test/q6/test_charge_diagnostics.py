...
from QligFEP import charge_diagnostics as diag, charge_probe as probe
...
def pre_observation_binary(tmp_path_factory):
    # Same source and solver as the observed executable; only trace calls differ.
    # The historical 4ad905d1 control used defective SHAKE and is no longer a
    # valid observer-only comparison after integration of the repaired solver.
    compiler = shutil.which('gfortran-11') or shutil.which('gfortran')
    directory = tmp_path_factory.mktemp('unobserved-native')/'source'
    shutil.copytree(PROJECT_ROOT/'src/q6', directory,
                    ignore=shutil.ignore_patterns('*.o', '*.mod', 'qdyn', 'qprep', 'test_lincs', 'test_settle'))
    subprocess.run(['make', '-j1', '-C', str(directory), 'qdyn', 'FC='+compiler,
                    'FPP_FLG=-cpp -DQ_TEST_DISABLE_CHARGE_TRACE'],
                   capture_output=True, text=True, check=True, timeout=60)
    return directory/'qdyn'
