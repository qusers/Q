...
class TestSeriesCommands:
    ...
    def test_the_preparation_packs_solvent_at_the_mutation_protocol_distance(self, tmp_path):
        # qprep_prot's own default of 3.0 forbids water at hydrogen-bonding distance,
        # which empties the first shell around the side chain being perturbed.
        series = self.series(tmp_path)
        command = series._qprep_command(LEU39_CB)
        assert command[command.index("-sp") + 1] == "2.5"

    def test_the_preparation_does_not_neutralize_included_salt_bridge_partners(self, tmp_path):
        """Charges inside the simulated region remain active even when their
        salt-bridge partner lies beyond the spherical boundary."""
        series = self.series(tmp_path)
        command = series._qprep_command(LEU39_CB)
        assert command[command.index("-sbc") + 1] == "0"
