...
__global__ void calc_ww(const int N_ITERATION, int N, double crg_ow,
                        double crg_hw, double A_OO, double B_OO, topo_t D_topo,
                        coord_t *__restrict__ W, dvel_t *__restrict__ DV_W,
                        double *Evdw_TOT, double *ecoul_TOT) {
  int lane_id = threadIdx.x % warpSize;

  int j = blockDim.x * blockIdx.x + threadIdx.x;
  int ibeg = N_ITERATION * blockIdx.y;
  //  int ilimit = min(ibeg + N_ITERATION, N);
  int ilimit = ibeg + N_ITERATION;

  double evdw = 0;  // useful
  double ecoul = 0; // useful
  double evdw_tot = 0, ecoul_tot = 0;
  double v_a = 0, v_b = 0;

  double v_x = 0, v_y = 0, v_z = 0; // useful
  coord_t tmp;
  tmp.x = tmp.y = tmp.z = 0;

  __shared__ coord_t rows[32];
  // read rows
  if (ibeg + lane_id < N) {
    rows[lane_id] = W[ibeg + lane_id];
  }
  __shared__ double sum_row_x[32];
  __shared__ double sum_row_y[32];
  __shared__ double sum_row_z[32];
  sum_row_x[lane_id] = sum_row_y[lane_id] = sum_row_z[lane_id] = 0;
  __syncthreads();
  ...
  if (j < N) {
    ...
    for (int idx = ibeg; idx < ilimit; idx++) {
      int row_i = (idx - ibeg + lane_id) % 32;
      int i = ibeg + row_i;
      ecoul = evdw = 0;
      if (i < N) {
        double inv_dis = 0;
        int belong_i = i / 3;
        bool i_is_o = (i % 3 == 0);
        if (belong_i < belong_j) {

          //          tmp.x = W[j].x - W[i].x;
          //          tmp.y = W[j].y - W[i].y;
          //          tmp.z = W[j].z - W[i].z;
          tmp.x = W[j].x - rows[row_i].x;
          tmp.y = W[j].y - rows[row_i].y;
          tmp.z = W[j].z - rows[row_i].z;

          inv_dis = rsqrt(tmp.x * tmp.x + tmp.y * tmp.y + tmp.z * tmp.z);
          double inv_dis2 = inv_dis * inv_dis;
          // inv_dis = sqrt(1.0 / (pow(tmp.x, 2) + pow(tmp.y, 2) + pow(tmp.z, 2)));
          ecoul = inv_dis * D_topo.coulomb_constant *
                  (i_is_o ? crg_ow : crg_hw) * (j_is_o ? crg_ow : crg_hw);
          if (i_is_o && j_is_o) {
          ...
          double dv = inv_dis2 * (-ecoul - 12.0 * v_a + 6.0 * v_b);
          v_x = dv * tmp.x;
          v_y = dv * tmp.y;
          v_z = dv * tmp.z;
          j_x += v_x;
  ...
  if (lane_id == 0) {
    atomicAdd(Evdw_TOT, evdw_tot);
    atomicAdd(ecoul_TOT, ecoul_tot);

    for (int i = ibeg; i < ilimit; i++) {
      atomicAdd(&DV_W[i].x, sum_row_x[i - ibeg]);
      atomicAdd(&DV_W[i].y, sum_row_y[i - ibeg]);
      atomicAdd(&DV_W[i].z, sum_row_z[i - ibeg]);
    }
  }

  atomicAdd(&DV_W[j].x, j_x);
  atomicAdd(&DV_W[j].y, j_y);
  atomicAdd(&DV_W[j].z, j_z);
}
...
void calc_nonbonded_ww_forces_host() {
  ...
  */
  ...
  int thread_num = 32;
  dim3 block_sz = dim3(thread_num, 1);


  const int N_ITERATION = 32;
  int grid_sz_x = (N + thread_num - 1) / thread_num;
  int grid_sz_y = (N + N_ITERATION - 1) / N_ITERATION;
  dim3 grid = dim3(grid_sz_x, grid_sz_y);

  calc_ww<<<grid, block_sz>>>(N_ITERATION, N, crg_ow, crg_hw, A_OO, B_OO, topo,
                              W, DV_W, D_WW_evdw_TOT, D_WW_ecoul_TOT);
