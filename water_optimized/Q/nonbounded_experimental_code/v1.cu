  ...
  if (j < N) {
    ...
    for (int i = ibeg; i < ilimit; i++) {
      ...
      if (belong_i < belong_j) {
        ...
        ecoul = inv_dis * D_topo.coulomb_constant * (i_is_o ? crg_ow : crg_hw) *
                (j_is_o ? crg_ow : crg_hw);
        if (i_is_o && j_is_o) {
          v_a = A_OO * pow(inv_dis, 12);
          v_b = B_OO * pow(inv_dis, 6);
          evdw = v_a - v_b;
        } else {
          v_a = 0;
          v_b = 0;
        ...
        double dv = inv_dis * inv_dis * (-ecoul - 12.0 * v_a + 6.0 * v_b);
        v_x = dv * tmp.x;
        v_y = dv * tmp.y;
        v_z = dv * tmp.z;
        j_x += v_x;
      ...
      for (unsigned w = 16; w >= 1; w /= 2) {
        ecoul += __shfl_down_sync(0xffffffff, ecoul, w);
        evdw += __shfl_down_sync(0xffffffff, evdw, w);
        v_x += __shfl_down_sync(0xffffffff, v_x, w);
        v_y += __shfl_down_sync(0xffffffff, v_y, w);
        v_z += __shfl_down_sync(0xffffffff, v_z, w);
      }
      if (lane_id == 0) {
        evdw_tot += evdw;
        ecoul_tot += ecoul;
        atomicAdd(&DV_W[i].x, -v_x);
        atomicAdd(&DV_W[i].y, -v_y);
        atomicAdd(&DV_W[i].z, -v_z);
      }
    }
