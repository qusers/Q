...
#pragma unroll
    for (int i = 0; i < Block_x; i++) {
        ...
        const coord_t &now_p0 = W[x];
        const coord_t &now_p1 = W[N - x];
#pragma unroll
...
void calc_nonbonded_ww_forces_host() {
    ...
    */
    ...
    const int thread_num_x = 16;  // Keep at 32 for better warp utilization
    const int thread_num_y = 4;   // Keep at 1 for better memory coalescing
    dim3 block_sz = dim3(thread_num_x, thread_num_y);

    const int N_ITERATION_Y = 64;  // Keep at 32 for better memory access pattern
